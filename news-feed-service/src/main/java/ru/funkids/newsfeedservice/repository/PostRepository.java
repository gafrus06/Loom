...
public interface PostRepository extends JpaRepository<Post, UUID> {
    ...
    Page<Post> findByCampId(@Param("campId") UUID campId, Pageable pageable);
    ...
    Page<Post> findByCampIdByDateDesc(@Param("campId") UUID campId, Pageable pageable);

    @Query("SELECT p FROM Post p WHERE p.detachmentId = :detachmentId " +
            "ORDER BY p.pinned DESC, p.pinnedOrder DESC NULLS LAST, p.createdAt DESC")
    Page<Post> findByDetachmentId(@Param("detachmentId") UUID detachmentId, Pageable pageable);
